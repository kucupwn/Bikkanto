...
export function EditExerciseModal({
  ...
  const [editedExercise, setEditedExercise] = useState<Exercise>(exercises[0]);
  ...
  function handleEditExercise() {
    const existing = exercises.find(
      (ex) => ex.exercise_name === editedExercise.exercise_name.toLowerCase(),
    );

    if (existing) {
      console.error(`${editedExercise.exercise_name} already exists.`);
    } else {
      ...
      <ModalBox onClick={(e) => e.stopPropagation()}>
        ...
        <ExercisePropsContainer>
          <InputWrapper>
            ...
            <select
              name="category_id"
              value={editedExercise.category_id}
              onChange={handleChange}
            >
              {categories.map((cat) => (
          ...
          <InputWrapper>
            ...
            <input
              name="medium_min"
              type="number"
              value={editedExercise.medium_min}
              onChange={handleChange}
            />
          </InputWrapper>
          <InputWrapper>
            ...
            <input
              name="medium_max"
              type="number"
              value={editedExercise.medium_max}
              onChange={handleChange}
            />
          </InputWrapper>
          <InputWrapper>
          ...
          <InputWrapper>
            ...
            <input
              name="hard_max"
              type="number"
              value={editedExercise.hard_max}
              onChange={handleChange}
            />
          </InputWrapper>
        </ExercisePropsContainer>
        <Button onClick={handleEditExercise}>Add</Button>
      </ModalBox>
    </Overlay>
  );
}
